...
export function validChart(): BaziChart {
  return {
    ...
    // 命局神煞（日干庚、年支午、日支辰、月支巳，手算结果，年支+日支双查）：
    //   吉神：天乙贵人(庚→丑未，时柱未)、福星贵人(庚→午，年柱午)、学堂(庚→巳，月柱巳)、
    //         华盖(日支辰→辰，日柱辰)、将星(年支午→午，年柱午)、
    //         月德贵人(月支巳→庚，四柱天干庚命中年柱+日柱)、天德贵人(月支巳→辛，月柱辛)
    //   凶煞：劫煞(日支辰→巳，月柱巳)、亡神(年支午→巳，月柱巳)、灾煞(日支辰→午，年柱午)、
    //         孤辰寡宿(年支午→申辰→日柱辰；日支辰→巳丑→月柱巳)、魁罡(庚辰∈集合)
    shenSha: {
      auspicious: [
