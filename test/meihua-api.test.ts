...
import type { MeihuaEnv } from "../src/meihua/types";
import { validRequest } from "./fixtures/meihua-request";
...
const baseEnv: MeihuaEnv = {
  LLM_BASE_URL: "https://apihub.agnes-ai.cn",
  LLM_MODEL: "agnes-2.0-flash",
  LLM_API_KEY: "test-key",
};

function allowLimiter(success: boolean): MeihuaEnv["MEIHUA_RATE_LIMITER"] {
  return { limit: async () => ({ success }) };
}

function req(body: unknown): Request {
  return new Request("http://localhost/api/meihua/interpret", {
    method: "POST",
...
describe("POST /api/meihua/interpret", () => {
  it("returns markdown on success", async () => {
    fetchMock
      .get("https://apihub.agnes-ai.cn")
      .intercept({ path: "/v1/chat/completions", method: "POST" })
      .reply(200, { choices: [{ message: { content: "## 卦象解读\n内容" } }] });
    const res = await api.fetch(req(validRequest()), { ...baseEnv, MEIHUA_RATE_LIMITER: allowLimiter(true) });
  ...
  it("returns 429 when rate limited", async () => {
    const res = await api.fetch(req(validRequest()), { ...baseEnv, MEIHUA_RATE_LIMITER: allowLimiter(false) });
