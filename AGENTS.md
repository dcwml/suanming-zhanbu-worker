...
src/
  ...
  layout/nav.ts       品牌块（logo.png + 站名）+ 导航（含「占卜」下拉：DIVINATION_NAV_LABEL/DIVINATION_NAV_ITEMS，六爻起卦/梅花易数；「运势」下拉：FORTUNE_NAV_LABEL/FORTUNE_NAV_ITEMS，每日/每周/每月运势；两个下拉均纯 CSS）+ 语言切换
...
public/assets/        静态资源（style.css、logo.png（印章 LOGO，兼作 favicon）、og-default.png、bazi.js、liuyao.js、meihua.js、zeji.js），由 Workers assets 直接服务；bazi/liuyao/meihua/zeji 页面经 CDN 统一加载 lunar-javascript 1.7.7（cdnjs 主源 + staticfile 回退）
...
test/                 27 个测试文件、303 个测试（SELF.fetch 集成测试 + 单元测试）
```
