...
脚本自动完成：读当日 almanac 数据，经 prompt 规则引擎（`src/cover/prompt.ts`）组装画面——**生肖主角（外形特征表防画错）× 吉凶情绪**（黄道吉日金光朝霞、凶神化解日阴转多云、晦日阴雨暮色）**× 纳音五行场景**（沙中金→鎏金石滩、天河水→烟雨溪涧）**× 确定性随机构图**（近景/远景/月下等，同日重跑不变、跨日错开）→ 调 Agnes 生图（1K 16:9）→ 压缩主图 jpg（1312×736）+ 归档缩略图 webp → 上传 R2 `covers/daily/` 前缀（复用本机 wrangler 登录态，无需额外凭证）。
