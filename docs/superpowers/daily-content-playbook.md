...
脚本自动完成：读当日 almanac 数据，经 prompt 规则引擎（`src/cover/prompt.ts`）组装画面——**画风**（天干五行 → 金碧山水/木刻版画/水墨写意/敦煌壁画/浅绛山水五画种）**× 生肖主角**（外形特征表防画错）**× 吉凶情绪**（黄道吉日金光朝霞、凶神化解日阴转多云、晦日阴雨暮色）**× 纳音五行场景**（沙中金→鎏金石滩、天河水→烟雨溪涧，与画风层的天干五行相互独立）**× 确定性随机构图**（近景/远景/月下等，同日重跑不变、跨日错开）→ 调 Agnes 生图（1K 16:9）→ 压缩主图 jpg（1312×736）+ 归档缩略图 webp → 上传 R2 `covers/daily/` 前缀（复用本机 wrangler 登录态，无需额外凭证）。

**生成后必须人工看图**：
- 主角生肖与当日 `zodiac` 字段一致（仍画错就重跑一次）
- **画面情绪与当日吉凶相符**（吉日应喜庆明亮、晦日应阴郁低沉，脚本日志会打出 情绪=吉/化解/晦）
- **画风与当日天干五行相符**（脚本日志会打出 画风=金碧山水/木刻版画/水墨写意/敦煌壁画/浅绛山水；木刻、敦煌两档易诱导文字，重点看有无字）
- 画面无文字、无乱码、无明显畸变
