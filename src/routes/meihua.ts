...
import { buildSystemPrompt, buildUserPrompt } from "../meihua/prompt";
import type { MeihuaEnv } from "../meihua/types";
import { validateInterpretRequest } from "../meihua/validate";
...
/** 注册梅花易数解读路由（在 api 子应用内，basePath 已是 /api） */
export function registerMeihuaRoutes(api: Hono<{ Bindings: MeihuaEnv & StatsEnv }>): void {
  api.post("/meihua/interpret", async (c) => {
    // 0. 记录 API 调用（异步，不阻塞主流程）
    const db = c.env?.STATS_DB;
    if (db) {
      recordApiCall(db, "/api/meihua/interpret").catch(() => {});
    }

    // 1. 限流（绑定缺失则跳过，本地 dev / 测试环境可用）
    const limiter = c.env?.MEIHUA_RATE_LIMITER;
    if (limiter) {
